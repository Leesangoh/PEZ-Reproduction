...
def get_transform():
    """Preprocessing pipeline: resize to model input size, normalize with ImageNet stats."""
...
def load_video_frames(video_dir, transform):
    """Load 16 PNG frames, apply transform, return (16, 3, H, W)."""
    frames = []
    for i in range(N_FRAMES):
        ...
        tensor = transform(tensor)  # (3, H, W) float32 normalized
        frames.append(tensor)
    return torch.stack(frames)  # (16, 3, H, W)
...
def extract_features(model, video_dirs, transform, n_layers, embed_dim, batch_size, device):
    ...
    for batch_start in tqdm(range(0, n_videos, batch_size), desc="  Extracting"):
        ...
        for vid_idx in range(batch_start, batch_end):
            ...
            clip = frames.permute(1, 0, 2, 3)  # (3, 16, H, W)
        ...
        if not token_shape_checked:
            ...
            expected_tokens = (N_FRAMES // 2) * ((VJEPA2_INPUT_SIZE // 16) ** 2)
            ...
            assert token_shape[1] == expected_tokens, (
                ...
                f"Check input resolution (should be {VJEPA2_INPUT_SIZE}x{VJEPA2_INPUT_SIZE})."
            )
